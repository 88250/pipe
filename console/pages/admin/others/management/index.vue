<template>
  <div>
    <div class="card fn-clear card__body card--space" v-if="$store.state.role <= 1">
      ...
      <v-btn class="btn btn--info">JSON</v-btn>
      <v-btn class="btn btn--info btn--space">{{ $t('staticBlog', $store.state.locale) }}</v-btn>
    </div>

    <div class="card fn-clear card__body card--space">
      <h1>{{ $t('export', $store.state.locale) }}</h1>
      <v-btn class="btn btn--info">JSON</v-btn>
      <v-btn class="btn btn--info btn--space">HTML</v-btn>
    </div>

    <div class="card fn-clear card__body card--space">
      <h1>{{ $t('clearData', $store.state.locale) }}</h1>
      <v-btn class="btn btn--danger">{{ $t('removeUnusedTags', $store.state.locale) }}</v-btn>
      <v-btn class="btn btn--danger btn--space">{{ $t('removeData', $store.state.locale) }}</v-btn>
    </div>
